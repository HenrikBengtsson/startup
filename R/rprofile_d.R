...
#' @aliases rprofile
#' @export
rprofile_d <- function(sibling = FALSE, all = FALSE, unload = FALSE, skip = NA, on_error = c("error", "warning", "immediate.warning", "message", "ignore"), dryrun = NA, debug = NA, paths = NULL) {
  debug <- debug(debug)
  ...
  if (!skip) {
    ...
    files_apply(files, FUN = FUN, on_error = on_error, dryrun = dryrun, what = "Rprofile")
